...
void DGDiffusionMIPIntegrator::AssembleFaceMatrix(
   const FiniteElement &el1, const FiniteElement &el2,
   FaceElementTransformations &Trans, DenseMatrix &elmat)
{
   int dim, ndof1, ndof2, ndofs, elOrder;
   bool kappa_is_nonzero = (kappa != 0.);
   double w, wq = 0.0;
   double compare = 0.25;
   double MIPcompare = 0.0;
   ...
   for (int p = 0; p < ir->GetNPoints(); p++)
   {
      ...
      //std::cout <<" Trans.Elem1->Weight: " << Trans.Elem1->Weight() << "\n";
      MIPcompare = compare*ip.weight*pow(nor*nor/(Trans.Elem1->Weight()*Trans.Elem1->Weight()), 1.0/dim)*Trans.Elem1->Weight();
      elOrder = el1.GetOrder();
      //std::cout << w << "\n";
      if (ndof2) // interior surface
      {
         w /= 2;
         MIPcompare /=2;
      }
      ...
      CalcAdjugate(Trans.Elem1->Jacobian(), adjJ);
      //adjJ.Print(std::cout, 8);
      adjJ.Mult(ni, nh);
      if (kappa_is_nonzero)
      {
         // Multiply wq by Cp(p+1) for element 1
         wq = ni * nor*elOrder*(elOrder+1);
            //std::cout << "h=1/sqrt(nor.nor/J1^2):\t" << 1/pow(nor*nor/(Trans.Elem1->Weight()*Trans.Elem1->Weight()), 1.0/dim) << "\n";
         //std::cout << "1:\t" << wq << "\n";
      }
      // Note: in the jump term, we use 1/h1 = |nor|/det(J1) which is
      // independent of Loc1 and always gives the size of element 1 in
      ...
      if (ndof2) // true for interior surfaces
      {
         Trans.Loc2.Transform(ip, eip2);
         el2.CalcShape(eip2, shape2);
         el2.CalcDShape(eip2, dshape2);
         Trans.Elem2->SetIntPoint(&eip2);
         w = ip.weight/2/Trans.Elem2->Weight();
         MIPcompare += compare*ip.weight*pow(nor*nor/(Trans.Elem2->Weight()*Trans.Elem2->Weight()), 1.0/dim)*Trans.Elem2->Weight()/2;
         elOrder = el2.GetOrder();
         if (!MQ)
         {
            if (Q)
            {
         ...
         if (kappa_is_nonzero)
         {
            // Multiply wq by Cp(p+1) for element 2
            //std::cout << "2:\t" << wq << "\n";
            wq += ni * nor*elOrder*(elOrder+1);
            //std::cout << "h=1/sqrt(nor.nor/J2^2):\t" << 1/pow(nor*nor/(Trans.Elem2->Weight()*Trans.Elem2->Weight()), 1.0/dim) << "\n";
            //std::cout << "2:\t" << wq << "\n";
         }
      ...
      if (kappa_is_nonzero) // true
      {
         // only assemble the lower triangular part of jmat
         wq *= kappa; // "weighted Q" ?? wq is almost kappa*{Q/h}
         //std::cout << "wq: " << wq << "\n";
         //std::cout << "MIPcompare: " << MIPcompare << "\n";
         // TODO: I think wq is where the MIP magic happens.
         //       (Hopefully) Cp(p+1) is multiplied into wq for each 
         //       side of the surface above. Here, need to take 
         //       the max(wq, 1/4)
         for (int i = 0; i < ndof1; i++)
         {
            //const double wsi = wq*shape1(i);
            ...
            if (ndof2)
            {
               wsi = std::max(wq, MIPcompare)*shape1(i);
            }
            else
            {
               if (MIPcompare > wq)
               {
                  wsi = (MIPcompare)*shape1(i);
               }
               else
               {
                  wsi = wq*shape1(i);
               }
               //wsi = std::max(wq, 0.25/16)*shape1(i);
            }
            for (int j = 0; j <= i; j++)
            {
               jmat(i, j) += wsi * shape1(j); // actually build jmat
            }
         }
         if (ndof2) // true for interior surfaces
         {
            for (int i = 0; i < ndof2; i++)
            {
               const int i2 = ndof1 + i;
               //const double wsi = wq*shape2(i);
               double wsi = std::max(wq, MIPcompare)*shape2(i);
               for (int j = 0; j < ndof1; j++)
               {
                  jmat(i2, j) -= wsi * shape1(j);
               }
...
void DGCurrentIntegrator::AssembleFaceMatrix(
   const FiniteElement &el1, const FiniteElement &el2,
   FaceElementTransformations &Trans, DenseMatrix &elmat)
{
#if 0 // Old way
   
   std::cout << "NOT MIP DSA\n";
   ...
   // elmat := 0*sigma*elmat^t - 0*elmat + jmat
   // elmat := jmat
   if (kappa_is_nonzero)
   {
      for (int i = 0; i < ndofs; i++)
      {
         ...
         {
            double aij = elmat(i,j), aji = elmat(j,i), mij = jmat(i,j);
            elmat(i,j) = 0*sigma*aji - 0*aij + mij;
            elmat(j,i) = 0*sigma*aij - 0*aij + mij;
         }
         elmat(i,i) = (0*sigma - 0*1.)*elmat(i,i) + jmat(i,i);
      }
   }
   else
   {
...
#if 0 // Method 1
   // elmat := -0*elmat + sigma*elmat^t + jmat
   if (kappa_is_nonzero)
   {
      for (int i = 0; i < ndofs; i++)
      {
         ...
         {
            double aij = elmat(i,j), aji = elmat(j,i), mij = jmat(i,j);
            elmat(i,j) = sigma*aji - 0*aij + mij;
            elmat(j,i) = sigma*aij - 0*aji + mij;
         }
         elmat(i,i) = (sigma - 0*1.)*elmat(i,i) + jmat(i,i);
      }
   }
   else
   {
...
#if 1 // Method 2
   // elmat := -0*elmat + 0*sigma*elmat^t + jmat
   if (kappa_is_nonzero)
   {
      for (int i = 0; i < ndofs; i++)
      {
         for (int j = 0; j < i; j++)
         {
            double aij = elmat(i,j), aji = elmat(j,i), mij = jmat(i,j);
            elmat(i,j) = 0*sigma*aji - 0*aij + mij;
            elmat(j,i) = 0*sigma*aij - 0*aji + mij;
         }
         elmat(i,i) = (0*sigma - 0*1.)*elmat(i,i) + jmat(i,i);
      }
   }
