...
/** Integrator for the Modified Interior Penalty (MIP) DG form from Wang and Ragusa (2010):
\f[
    - < {(Q grad(u)).n}, [v] > + sigma < [u], {(Q grad(v)).n} >
    + kappa < {h^{-1} Q} [u], [v] >,
\f]
    ...
    where
\f[
   \kappa_e = \max \left(\kappa_e^{IP}, \frac{1}{4} \right)
\f]
   is the stabilization parameter, where
\f[
   \kappa_e^{IP} =
   \begin{cases}
      \frac{c(p^+)}{2} \frac{D^+}{h_\perp^+} + \frac{c(p^-)}{2} \frac{D^-}{h_\perp^-}, & \text{on interior edges} \\
      c(p)\frac{D}{h_\perp}, & \text{on boundary edges} \\
   \end{cases}
\f]
    is the interior penalty method, where \f$c(p) = Cp(p+1)\f$, \f$C\f$ is a constant, \f$p\f$ is the polynomial order, \f$D\f$ is the diffusion coefficient, and \f$h_\perp\f$ is the length of the cell in the direction orthogonal to edge \f$e\f$.
    ...
    - sigma = +1, kappa > 0: non-symmetric interior penalty (NIPG) method,
    - sigma = +1, kappa = 0: the method of Baumann and Oden. */
class DGDiffusionMIPIntegrator : public BilinearFormIntegrator
{
protected:
   Coefficient *Q;
   MatrixCoefficient *MQ;
...
public:
   DGDiffusionMIPIntegrator(const double s, const double k)
      : Q(NULL), MQ(NULL), sigma(s), kappa(k) { }
   DGDiffusionMIPIntegrator(Coefficient &q, const double s, const double k)
      : Q(&q), MQ(NULL), sigma(s), kappa(k) { }
   DGDiffusionMIPIntegrator(MatrixCoefficient &q, const double s, const double k)
      : Q(NULL), MQ(&q), sigma(s), kappa(k) { }
   using BilinearFormIntegrator::AssembleFaceMatrix;
   virtual void AssembleFaceMatrix(const FiniteElement &el1,
                                   const FiniteElement &el2,
...
class DGCurrentIntegrator : public BilinearFormIntegrator
{
protected:
   Coefficient *Q;
   MatrixCoefficient *MQ;
...
public:
   DGCurrentIntegrator(const double s, const double k)
      : Q(NULL), MQ(NULL), sigma(s), kappa(k) { }
   DGCurrentIntegrator(Coefficient &q, const double s, const double k)
      : Q(&q), MQ(NULL), sigma(s), kappa(k) { }
   DGCurrentIntegrator(MatrixCoefficient &q, const double s, const double k)
      : Q(NULL), MQ(&q), sigma(s), kappa(k) { }
   using BilinearFormIntegrator::AssembleFaceMatrix;
   virtual void AssembleFaceMatrix(const FiniteElement &el1,
                                   const FiniteElement &el2,
